    ...
    await _firestore.runTransaction((transaction) async {
      ...
      if (items.isNotEmpty) {
        ...
        quantity = (currentData['itemCount'] ?? items.length) is int
            ? currentData['itemCount'] ?? items.length
            : int.tryParse('${currentData['itemCount'] ?? items.length}') ?? 0;
        unitPrice = (firstItem['unitPrice'] ?? 0).toDouble();
        totalPrice = (currentData['totalAmount'] ?? 0).toDouble();
      } else {
        ...
        unitPrice = (currentData['unitPrice'] ?? 0).toDouble();
        totalPrice = (currentData['totalAmount'] ?? currentData['totalPrice'] ?? 0)
            .toDouble();
      }
